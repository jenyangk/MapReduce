...
typedef struct {
    ...
    std::vector <ThreadPool_work_t*> queue; // Work queue
} ThreadPool_work_queue_t;

typedef struct {
    // TODO: Add members here
    std::vector<pthread_t*> mThreads; // Pool of threads
    pthread_cond_t work_arrived; // Work condition variable
    pthread_mutex_t mutex; // Mutex used for locking
    volatile bool mStoppping; // Stops the threadpool
    ThreadPool_work_queue_t* jobQueue; // Job queue
} ThreadPool_t;
